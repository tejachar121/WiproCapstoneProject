...
class DeliveryServiceTest {
    ...
    @Test
    void testAssignDelivery_AgentFound() {
        ...
        when(agentRepo.findById(2L))
                .thenReturn(Optional.of(agent));

        Delivery saved = new Delivery();
        saved.setOrderId(1L);

        when(deliveryRepo.save(any(Delivery.class)))
                .thenReturn(saved);

        Delivery result = deliveryService.assignDelivery(req);
